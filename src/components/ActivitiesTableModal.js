...
const ActivitiesTableModal = ({ activities, userId, onClose }) => {
  ...
  const handleActivitySelection = (activity) => {
    ...
    function parseReferralCode(link) {
      if (!link) return null;
      try {
        const url = new URL(link);
        // e.g. "REF-o5YPfYn9HfZGmSF0w8ET9MpMdhz2-1737375984311"
        return url.searchParams.get('ref');
      } catch (err) {
        console.error('Failed to parse referralLink:', err);
        return null;
      }
    }
...
const handleSubscribe = async () => {
  ...
  try {
    ...
    // 1) Parse the code from the selected referralLink
    const referralCode = parseReferralCode(selectedProject.referralLink);
    ...
    for (const activity of selectedActivities) {
      ...
      if (activitySnapshot.exists()) {
        ...
        // 2) Check if user already in this doc
        ...
        if (userAlreadyExists) {
          ...
          // 3) Update existing doc with user, referralLink, **and** referralCode
          await updateDoc(activityRef, {
            ...
            referralLink: selectedProject.referralLink || 'غير متوفر',
            referralCode: referralCode || '', // NEW: store the code
          });
          console.log(`تم تسجيل المستخدم في النشاط: ${activity.name}`);
        }
      } else {
        // 4) If doc doesn't exist, create it with referralLink + referralCode
